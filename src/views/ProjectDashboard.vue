<template>
  <!-- 小游戏项目使用专用看板 -->
  <MiniGamesDashboard v-if="isMiniGamesProject" />

  <!-- 其他项目使用通用看板 -->
  <div v-else class="project-dashboard">
    <!-- 项目头部信息 -->
    <el-card class="project-header" v-if="currentProject">
      <div class="header-content">
        <div class="project-info">
          <div class="project-icon">{{ currentProject.builtIn ? '🔥' : '📂' }}</div>
          <div class="project-details">
            <h2 class="project-title">
              {{ currentProject.name }}
              <el-tag v-if="currentProject.builtIn" type="danger" size="small" style="margin-left: 8px">
                官方
              </el-tag>
            </h2>
            <p class="project-desc">{{ currentProject.description || '暂无描述' }}</p>
          </div>
        </div>
        <div class="header-actions">
          ...
          />
          <div v-if="dateRange && dateRange.length === 2" class="date-range-info-inline">
            <span class="date-range-text">{{ dateRange[0].slice(5) }} 至 {{ dateRange[1].slice(5) }}</span>
            <el-tag type="success" size="small">
              {{ filteredEventsByDate.length }} 条
            </el-tag>
            <el-button text type="primary" size="small" @click="clearDateRange">
              清除
            </el-button>
          </div>
    ...
    <div v-else-if="error" class="error-state">
      ...
      <el-button type="primary" @click="loadData(true)">重试</el-button>
    </div>

    <!-- 数据内容 -->
    <template v-else-if="rawEvents.length > 0">
      <!-- 统计卡片 -->
    ...
    <div v-else class="empty-state">
      ...
      <el-button type="primary" @click="loadData(true)">刷新数据</el-button>
    </div>
  </div>
</template>
...
import MiniGamesDashboard from '@/views/MiniGamesDashboard.vue'
import { ElMessage } from 'element-plus'
import { Refresh, Clock, Calendar, Loading, DataLine, User, TrendCharts } from '@element-plus/icons-vue'
...
// 是否为小游戏项目
const isMiniGamesProject = computed(() => {
  return currentProject.value?.id === 'built_in_mini_games'
})
...
const loadData = async (forceRefresh = true) => {
  ...
  // 如果不强制刷新，先尝试从缓存加载
  if (!forceRefresh) {
    const cachedData = getCachedData(currentProject.value.id)
    if (cachedData) {
  ...
  try {
    ...
    // 检测并转换小游戏项目的数据格式
    // 小游戏使用 { t: 'type', ts: timestamp, d: data } 格式
    let events = data?.events || []
    let topLevelUserId = data?.u || null // 提取顶层用户ID

    // 检测是否为小游戏格式（第一个事件是否有 t 字段）
    ...
    analytics.value = analyzeData(rawEvents.value, topLevelUserId)
    ...
    // 保存到缓存
    saveCachedData(currentProject.value.id, {
...
// 分析数据
const analyzeData = (events, topLevelUserId = null) => {
...
watch(() => route.params.projectId, async (newProjectId) => {
  if (newProjectId) {
    ...
    if (project) {
      ...
      document.title = `${project.name} - 数据看板`
